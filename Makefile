...
# 
FILE_DA_CONSEGNARE=membox.c membox.h connections.c connections.h thread_pool.c thread_pool.h icl_hash.h icl_hash.c message.h ops.h stats.h DATA/membox.conf1 DATA/membox.conf2 DATA/membox.conf3 DATA/membox.conf4 testsave.sh testload.sh testmultithread.sh threadstress.sh client.c memboxstats.sh testleaks.sh testput.sh testlock.sh testsize.sh teststats.sh teststress.sh README_ISTRUCTION.txt RELAZIONE_PROGETTO.pdf gruppo-check.pl
# inserire il nome del tarball: es. NinoBixioGiuseppeGaribaldi
TARNAME=PandeleaAurora
#
###########################################################
...
.PHONY: all clean cleanall test1 test2 test3 test4 test5 test6 test7 test8 test9 consegna
.SUFFIXES: .c .h
...
cleanall	: clean
	\rm -f *.o *~ libmbox.a valgrind_out $(STAT_PATH) $(UNIX_PATH) $(SAVE_PATH)
...
test8:
	...
	./testsave.sh $(UNIX_PATH) $(STAT_PATH)
	killall -USR2 -w membox
	sleep 3
	./membox -f DATA/membox.conf3&
	./testload.sh $(UNIX_PATH) $(STAT_PATH)
	killall -USR2 -w membox
	@echo "********* test8 superato!"

	
# save-load file test 2
test9:
	make cleanall
	make all
	./membox -f DATA/membox.conf3&
...
# stress test multithread
test10:
	make cleanall
	make all
	./testmultithread.sh $(UNIX_PATH) DATA/membox.conf4 DATA/membox.conf3	
